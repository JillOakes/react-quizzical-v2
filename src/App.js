...
function App() {
  ...
  if (rawQuestions.length > 0) {
    for (let i = 0; i < rawQuestions.length; i++) {
      const foo = {
        id: i,
        question: htmlDecode(rawQuestions[i].question),
        answers: [
          {
            id: 5,
            text: htmlDecode(rawQuestions[i].correct_answer),
            isCorrect: true,
          },
          {
            id: 6,
            text: htmlDecode(rawQuestions[i].incorrect_answers[0]),
            isCorrect: false,
          },
          {
            id: 7,
            text: htmlDecode(rawQuestions[i].incorrect_answers[1]),
            isCorrect: false,
          },
          {
            id: 8,
            text: htmlDecode(rawQuestions[i].incorrect_answers[2]),
            isCorrect: false,
          },
        ],
      };
    ...
    <div className="App">
      <main>
        ...
        {showIntro ? (
          ...
          <Game gameArray={gameArray} setShowIntro={setShowIntro} />
        )}
      </main>
    </div>
  );
